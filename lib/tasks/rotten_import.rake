...
task :rotten_import => :environment do
  ...
  units.each do |unit|
    ...
    d["abridged_cast"].each do |act|
      artist = Artist.create(name: act["name"], rt_artist_id: act["id"])
      artist_movie = ArtistMovie.create(artist_id: artist.id, movie_id: movie.id)
      act["characters"].each do |c|
        character = Character.create(artist_id: artist.id, movie_id: movie.id, name: c)
      end
    end
  end
end